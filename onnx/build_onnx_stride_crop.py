...
def make_scalar_const(name, value, dtype=np.float32):
    ...
    return helper.make_tensor(
        name=name,
        data_type=TensorProto.FLOAT if dtype == np.float32 else TensorProto.INT64,
        dims=[1],
        vals=arr
    )
...
def build_algo_coeffs():
    # Input: Bayer image with symbolic NCHW (expect W=2048 at runtime; crop will reduce to 1920)
    bayer_in = helper.make_tensor_value_info('input.bayer', TensorProto.FLOAT, ['N','C','H','W'])

    # Dynamic crop parameters passed at runtime: [start_h, start_w], [end_h, end_w], [2,3]
    ...
    # --- Remove stride/padding by dynamic Slice to FHD
    ...
    # --- RGB proxy: replicate cropped Bayer to 3 channels
    nodes.append(helper.make_node(
        'Concat',
        ['bayer_cropped', 'bayer_cropped', 'bayer_cropped'],
        ['RGB'],
        axis=1
    ))

    # --- Luminance Y = M709 * RGB
    ...
    # Slice R,G,B per channel dimension
    ...
    graph = helper.make_graph(
        nodes,
        'ISP_Algo_Coeffs_StrideCrop_Dynamic',
        [bayer_in, crop_starts, crop_ends, crop_axes],
        outs,
        inits
    )
...
if __name__ == "__main__":
    ...
    onnx.save(model, "isp_onnx_stride_crop.onnx")
    print("Saved isp_onnx_stride_crop.onnx")
