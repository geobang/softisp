...
class ToneFilmicV1(ToneMapBase):
    ...
    def _provide_static_coeffs(self, stage, inits, vis):
        ...
        coeffs_vals = [0.2, 0.85, 1.9, 1.0, 0.2, 0.85, 1.85, 1.0, 0.2, 0.85, 2.0, 1.0]
        inits.append(oh.make_tensor(name=coeffs_name, data_type=TensorProto.FLOAT, dims=[3, 4], vals=coeffs_vals))
    ...
    def _gather_channel_coeffs(self, stage, coeffs_name, ch_idx, nodes):
        ...
        knee = f'{base}.knee'
        shoulder = f'{base}.shoulder'
        gamma = f'{base}.gamma'
        gain = f'{base}.gain'
        idx_knee = f'{base}.idx_knee'
        idx_shoulder = f'{base}.idx_shoulder'
        idx_gamma = f'{base}.idx_gamma'
        idx_gain = f'{base}.idx_gain'
        nodes.append(oh.make_node('Constant', inputs=[], outputs=[idx_knee], value=oh.make_tensor(idx_knee, TensorProto.INT64, [2], [ch_idx, 0])))
        nodes.append(oh.make_node('Constant', inputs=[], outputs=[idx_shoulder], value=oh.make_tensor(idx_shoulder, TensorProto.INT64, [2], [ch_idx, 1])))
        nodes.append(oh.make_node('Constant', inputs=[], outputs=[idx_gamma], value=oh.make_tensor(idx_gamma, TensorProto.INT64, [2], [ch_idx, 2])))
        nodes.append(oh.make_node('Constant', inputs=[], outputs=[idx_gain], value=oh.make_tensor(idx_gain, TensorProto.INT64, [2], [ch_idx, 3])))
        nodes.append(oh.make_node('GatherND', inputs=[coeffs_name, idx_knee], outputs=[knee], name=f'{base}.gather_knee'))
        nodes.append(oh.make_node('GatherND', inputs=[coeffs_name, idx_shoulder], outputs=[shoulder], name=f'{base}.gather_shoulder'))
        nodes.append(oh.make_node('GatherND', inputs=[coeffs_name, idx_gamma], outputs=[gamma], name=f'{base}.gather_gamma'))
        nodes.append(oh.make_node('GatherND', inputs=[coeffs_name, idx_gain], outputs=[gain], name=f'{base}.gather_gain'))
    ...
    def _apply_tone_rgb(self, stage, input_image, coeffs_name, nodes, vis):
        ...
        g = f'{stage}.g'
        b = f'{stage}.b'
        nodes.append(oh.make_node('Split', inputs=[input_image], outputs=[r, g, b], name=f'{stage}.split_rgb', axis=1))
        vis += [oh.make_tensor_value_info(r, TensorProto.FLOAT, ['n', 1, 'h', 'w']), oh.make_tensor_value_info(g, TensorProto.FLOAT, ['n', 1, 'h', 'w']), oh.make_tensor_value_info(b, TensorProto.FLOAT, ['n', 1, 'h', 'w'])]
        ...
        nodes.append(oh.make_node('Concat', inputs=[r_t, g_t, b_t], outputs=[applier], name=f'{stage}.concat_rgb', axis=1))
    ...
    def _mean_2d_then_n(self, stage, x, tag, nodes):
        ...
        m = f'{stage}.{tag}.m'
        nodes.append(oh.make_node('ReduceMean', inputs=[x], outputs=[m_hw], name=f'{stage}.{tag}.mean_hw', axes=[2, 3], keepdims=0))
        nodes.append(oh.make_node('ReduceMean', inputs=[m_hw], outputs=[m], name=f'{stage}.{tag}.mean_n', axes=[0], keepdims=0))
        return m
    ...
    def _calc_ci(self, stage, applier, nodes, vis):
        ...
        g = f'{stage}.ci_g'
        b = f'{stage}.ci_b'
        nodes.append(oh.make_node('Split', inputs=[applier], outputs=[r, g, b], name=f'{stage}.ci_split', axis=1))
        ...
        nodes.append(oh.make_node('Constant', inputs=[], outputs=[inv3], value=oh.make_tensor(inv3, TensorProto.FLOAT, [], [1.0 / 3.0])))
        nodes.append(oh.make_node('Mul', inputs=[sum_rgb, inv3], outputs=[white_mean], name=f'{stage}.white_mean'))
        ci_r = f'{stage}.ci_r_vec'
        ci_g = f'{stage}.ci_g_vec'
        ci_b = f'{stage}.ci_b_vec'
        ci_r_abs = f'{stage}.ci_r_abs'
        ci_g_abs = f'{stage}.ci_g_abs'
        ci_b_abs = f'{stage}.ci_b_abs'
        ...
        nodes.append(oh.make_node('Concat', inputs=[ci_r_abs, ci_g_abs, ci_b_abs], outputs=[ci_vec], name=f'{stage}.ci_vec', axis=0))
        vis.append(oh.make_tensor_value_info(ci_vec, TensorProto.FLOAT, [3]))
        ci_scalar = f'{stage}.ci_scalar'
        nodes.append(oh.make_node('ReduceMean', inputs=[ci_vec], outputs=[ci_scalar], name=f'{stage}.ci_scalar', axes=[0], keepdims=0))
