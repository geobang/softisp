# onnx/microblocks/tone/tone_filmic_v3.py
...
class ToneFilmicV3(ToneMapBase):
    ...
    version = "v3"
    deps = ["resize_base"]
    needs = ["input_image", "neutral_mask", "ci_weights"]  # only image + stats
    provides = ["applier", "tone_coeffs", "ci_vec", "ci_scalar"]

    # -----------------------------
    # Phase 0: optional LUT/init
    # -----------------------------
    ...
    # -----------------------------
    # Phase 1: calculate tone coeffs
    # -----------------------------
    def _calc_tone_coeffs(self, stage, in_img, nodes, vis):
        tone_coeffs = f"{stage}.tone_coeffs"
        out_img = f"{stage}.tone_applied"

        # Example: identity mapping for now (replace with curve ops)
        nodes.append(oh.make_node("Identity", inputs=[in_img], outputs=[out_img],
                                  name=f"{stage}.identity_tone"))
        vis.append(oh.make_tensor_value_info(out_img, TensorProto.FLOAT, ["n","3","h","w"]))
        vis.append(oh.make_tensor_value_info(tone_coeffs, TensorProto.FLOAT, [3,4]))  # placeholder coeffs

        return out_img, tone_coeffs

    # -----------------------------
    # Phase 2: apply tone coeffs
    # -----------------------------
    def _apply_tone(self, stage, out_img, nodes, vis):
        applier = f"{stage}.applier"
        nodes.append(oh.make_node("Identity", inputs=[out_img], outputs=[applier],
                                  name=f"{stage}.identity_applier"))
        vis.append(oh.make_tensor_value_info(applier, TensorProto.FLOAT, ["n","3","h","w"]))
        return applier

    # -----------------------------
    # Phase 3: calculate tone-bias CI
    # -----------------------------
    def _calc_ci(self, stage, applier, in_mask, in_weights, nodes, vis):
        ci_vec = f"{stage}.ci_vec"
        ci_scalar = f"{stage}.ci_scalar"

        # Example placeholders: Identity ops for now
        nodes.append(oh.make_node("Identity", inputs=[applier], outputs=[ci_vec],
                                  name=f"{stage}.identity_ci_vec"))
        nodes.append(oh.make_node("Identity", inputs=[applier], outputs=[ci_scalar],
                                  name=f"{stage}.identity_ci_scalar"))
