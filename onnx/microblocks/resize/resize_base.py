...
class ResizeBase(MicroblockBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        node = oh.make_node(
            "Resize",
            inputs=[input_image, "", target_h, target_w],
            outputs=[out_name],
            name=f"{stage}_resize"
        )

        vis = [
            oh.make_tensor_value_info(input_image, oh.TensorProto.FLOAT, ["n","3","h","width"]),
            oh.make_tensor_value_info(target_h, oh.TensorProto.INT64, [1]),
            oh.make_tensor_value_info(target_w, oh.TensorProto.INT64, [1]),
