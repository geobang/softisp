...
class WhiteBalanceV1(AWBBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        mul_node = oh.make_node('Mul', [input_image, f'{stage}.wb_gains'], [wb_out], name=f'{stage}_apply_wb')
        vis = [oh.make_tensor_value_info(input_image, onnx.TensorProto.FLOAT, ['n', 'c', 'h', 'w']), oh.make_tensor_value_info(f'{stage}.wb_gains', onnx.TensorProto.FLOAT, [3]), oh.make_tensor_value_info(wb_out, onnx.TensorProto.FLOAT, ['n', 'c', 'h', 'w'])]
        outputs = {'applier': {'name': wb_out}}
        return BuildResult(outputs, [mul_node], [], vis).appendInput(f'{prev_stages[0]}.applier')
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        reduce_node = oh.make_node('ReduceMean', inputs=[image_in], outputs=[reduce_out], axes=[2, 3], keepdims=0, name=f'{stage}_reduce_mean')
        r_out = f'{stage}.avg_r'
        g_out = f'{stage}.avg_g'
        b_out = f'{stage}.avg_b'
        ...
        const_g = oh.make_node('Constant', inputs=[], outputs=[gain_g], value=oh.make_tensor('', onnx.TensorProto.FLOAT, [1], [1.0]), name=f'{stage}_const_g')
        wb_out = f'{stage}.wb_gains'
        concat_node = oh.make_node('Concat', [gain_r, gain_g, gain_b], [wb_out], axis=0, name=f'{stage}_concat')
        inits = [oh.make_tensor(f'{stage}.r_start', onnx.TensorProto.INT64, [1], [0]), oh.make_tensor(f'{stage}.r_end', onnx.TensorProto.INT64, [1], [1]), oh.make_tensor(f'{stage}.g_start', onnx.TensorProto.INT64, [1], [1]), oh.make_tensor(f'{stage}.g_end', onnx.TensorProto.INT64, [1], [2]), oh.make_tensor(f'{stage}.b_start', onnx.TensorProto.INT64, [1], [2]), oh.make_tensor(f'{stage}.b_end', onnx.TensorProto.INT64, [1], [3])]
        nodes = [reduce_node, slice_r, slice_g, slice_b, div_r, div_b, const_g, concat_node]
        vis = [oh.make_tensor_value_info(image_in, onnx.TensorProto.FLOAT, ['n', 'c', 'h', 'w']), oh.make_tensor_value_info(wb_out, onnx.TensorProto.FLOAT, [3])]
        mul_node = oh.make_node('Mul', [image_in, f'{stage}.wb_gains'], [applier], name=f'{stage}_apply_wb')
        nodes += [mul_node]
        outputs = {'applier': {'name': applier}, 'wb_gains': {'name': wb_out}}
