...
class AWBLuxV1(AWBBase):
    ...
    def _compute_channel_means(self, stage, r, g, b, nodes, vis):
        ...
        vis += [oh.make_tensor_value_info(mr, TensorProto.FLOAT, []), oh.make_tensor_value_info(mg, TensorProto.FLOAT, []), oh.make_tensor_value_info(mb, TensorProto.FLOAT, [])]
        return (mr, mg, mb)
    ...
    def _derive_gains(self, stage, mr, mg, mb, lux_scalar, nodes, vis, inits):
        ...
        eps = f'{stage}.eps'
        inits.append(oh.make_tensor(eps, TensorProto.FLOAT, [], [1e-06]))
        ...
        one = f'{stage}.one'
        zero = f'{stage}.zero'
        inits += [oh.make_tensor(k, TensorProto.FLOAT, [], [0.7]), oh.make_tensor(one, TensorProto.FLOAT, [], [1.0]), oh.make_tensor(zero, TensorProto.FLOAT, [], [0.0])]
        k_lux = f'{stage}.k_lux'
        ...
        one_g = f'{stage}.one_g'
        one_b = f'{stage}.one_b'
        inits += [oh.make_tensor(one_r, TensorProto.FLOAT, [], [1.0]), oh.make_tensor(one_g, TensorProto.FLOAT, [], [1.0]), oh.make_tensor(one_b, TensorProto.FLOAT, [], [1.0])]
        ...
        nodes.append(oh.make_node('Concat', inputs=[gain_r, gain_g, gain_b], outputs=[wb_gains], name=f'{stage}.concat_wb_gains', axis=0))
    ...
    def _apply_gains(self, stage, input_image, wb_gains, nodes, vis):
        ...
        nodes.append(oh.make_node('Mul', inputs=[input_image, wb_gains], outputs=[applier], name=f'{stage}_awb_mul'))
        vis += [oh.make_tensor_value_info(input_image, TensorProto.FLOAT, ['n', 3, 'h', 'w']), oh.make_tensor_value_info(wb_gains, TensorProto.FLOAT, [3]), oh.make_tensor_value_info(applier, TensorProto.FLOAT, ['n', 3, 'h', 'w'])]
        return applier
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        outputs = {'wb_gains': {'name': wb_gains}, 'applier': {'name': applier}}
