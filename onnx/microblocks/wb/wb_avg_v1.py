...
class WBAvgV1(AWBBase):
    ...
    def _build_channel_means(self, stage, prev_stage, nodes, inits, vis):
        ...
        print(f'check {stage}.mean_channels')
        nodes.append(oh.make_node('ReduceMean', inputs=[in_image], outputs=[mean_name], name=f'{stage}.reduce_mean', keepdims=0, axes=[2, 3]))
        r_mean, g1_mean, g2_mean, b_mean = (f'{stage}.r_mean', f'{stage}.g1_mean', f'{stage}.g2_mean', f'{stage}.b_mean')
        split_const = f'{stage}.split_sizes'
        inits.append(oh.make_tensor(split_const, TensorProto.INT64, [4], [1, 1, 1, 1]))
        r_mean, g1_mean, g2_mean, b_mean = (f'{stage}.r_mean', f'{stage}.g1_mean', f'{stage}.g2_mean', f'{stage}.b_mean')
        nodes.append(oh.make_node('Split', inputs=[mean_name, split_const], outputs=[r_mean, g1_mean, g2_mean, b_mean], name=f'{stage}.split_rggb', axis=0))
    ...
    def _build_wb_gains(self, stage, nodes, r_mean, g_mean_avg, b_mean):
        ...
        nodes.append(oh.make_node('Concat', [gain_r, gain_g, gain_b], [out_wb], name=f'{stage}.concat_gains', axis=0))
        return out_wb
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        outputs = {'applier': {'name': applier}, 'wb_gains': {'name': out_wb}, 'cct': {'name': out_cct}}
