...
class GammaBase(MicroblockBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        node = oh.make_node('Pow', inputs=[input_image, gamma], outputs=[out_name], name=f'{stage}_gamma')
        vis = [oh.make_tensor_value_info(input_image, oh.TensorProto.FLOAT, ['n', '3', 'target_h', 'target_w']), oh.make_tensor_value_info(gamma, oh.TensorProto.FLOAT, [1]), oh.make_tensor_value_info(out_name, oh.TensorProto.FLOAT, ['n', '3', 'target_h', 'target_w'])]
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        default_gamma = 2.2
        inits.append(oh.make_tensor(gamma, oh.TensorProto.FLOAT, [1], [default_gamma]))
        upstream = prev_stages[0] if prev_stages else stage
        input_image = f'{upstream}.applier'
        out_name = f'{stage}.applier'
        ...
        outputs = {'applier': {'name': out_name}, 'gamma_value': {'name': gamma}}
