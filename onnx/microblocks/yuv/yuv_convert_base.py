...
class YUVConvertBase(MicroblockBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        node = oh.make_node('MatMul', inputs=[input_image, matrix], outputs=[out_name], name=f'{stage}_yuvconvert')
        vis = [oh.make_tensor_value_info(input_image, oh.TensorProto.FLOAT, ['n', '3', 'target_h', 'target_w']), oh.make_tensor_value_info(matrix, oh.TensorProto.FLOAT, [3, 3]), oh.make_tensor_value_info(out_name, oh.TensorProto.FLOAT, ['n', '3', 'target_h', 'target_w'])]
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        default_matrix = [0.299, 0.587, 0.114, -0.147, -0.289, 0.436, 0.615, -0.515, -0.1]
        inits.append(oh.make_tensor(matrix, oh.TensorProto.FLOAT, [3, 3], default_matrix))
        upstream = prev_stages[0] if prev_stages else stage
        input_image = f'{upstream}.applier'
        out_name = f'{stage}.applier'
        ...
        outputs = {'applier': {'name': out_name}, 'rgb2yuv_matrix': {'name': matrix}}
