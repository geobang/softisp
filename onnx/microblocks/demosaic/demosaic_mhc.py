...
class DemosaicMHC(DemosaicBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        # Placeholder repeats; set concrete ints if known
        tile_repeats_name, tile_repeats_init = _tile_repeats_init(stage, repeats=[1,1,1])
        ...
        # 5) Green interpolation at R/B sites (SSA-safe names)
        ...
        nodes += [
            ...
            oh.make_node("Mul",  [G_at_R_conv, M_R],             [G_at_R_masked], name=f"{stage}_mask_G_at_R"),
            oh.make_node("Conv", [B_full, f"{stage}.K_mhc_g_b"], [G_at_B_conv],   name=f"{stage}_conv_G_at_B"),
            oh.make_node("Mul",  [G_at_B_conv, M_B],             [G_at_B_masked], name=f"{stage}_mask_G_at_B"),
        ...
        nodes += [
            ...
            oh.make_node("Mul",  [R_at_B_conv, M_B],                 [R_at_B_masked],   name=f"{stage}_mask_R_at_B"),
            oh.make_node("Conv", [G_full_native, f"{stage}.K_mhc_r_g_h"], [R_at_G_h_conv], name=f"{stage}_conv_R_at_G_h"),
            oh.make_node("Mul",  [R_at_G_h_conv, M_G],               [R_at_G_h_masked], name=f"{stage}_mask_R_at_G_h"),
            oh.make_node("Conv", [G_full_native, f"{stage}.K_mhc_r_g_v"], [R_at_G_v_conv], name=f"{stage}_conv_R_at_G_v"),
            oh.make_node("Mul",  [R_at_G_v_conv, M_G],               [R_at_G_v_masked], name=f"{stage}_mask_R_at_G_v"),
        ...
        nodes += [
            ...
            oh.make_node("Mul",  [B_at_R_conv, M_R],                 [B_at_R_masked],   name=f"{stage}_mask_B_at_R"),
            oh.make_node("Conv", [G_full_native, f"{stage}.K_mhc_b_g_h"], [B_at_G_h_conv], name=f"{stage}_conv_B_at_G_h"),
            oh.make_node("Mul",  [B_at_G_h_conv, M_G],               [B_at_G_h_masked], name=f"{stage}_mask_B_at_G_h"),
            oh.make_node("Conv", [G_full_native, f"{stage}.K_mhc_b_g_v"], [B_at_G_v_conv], name=f"{stage}_conv_B_at_G_v"),
            oh.make_node("Mul",  [B_at_G_v_conv, M_G],               [B_at_G_v_masked], name=f"{stage}_mask_B_at_G_v"),
     ...
     #       oh.make_tensor_value_info(cfa_onehot, TensorProto.INT64, [2,2,4"]),
        ...
        result.appendInput(cfa4)        # CFA planes
