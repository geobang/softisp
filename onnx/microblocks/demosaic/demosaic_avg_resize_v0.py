...
class DemosaicAvgResizeV0(DemosaicBase):
    ...
    def _concat_rgb(self, stage, r, gavg, b, nodes, vis):
        ...
        nodes.append(oh.make_node("Concat", inputs=[r,gavg,b], outputs=[local_rgb],
    ...
    def _resize_half(self, stage, rgb, nodes, vis, inits):
        ...
        nodes.append(
            oh.make_node(
                "Resize",
                inputs=[rgb, scales],  # omit roi if not used
                outputs=[applier],
                name=f"{stage}.resize_half",
                mode="linear",  # "box" only if runtime supports
            )
        )
