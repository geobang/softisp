...
def make_bayer_frame(h=1080, w=2048, seed=None):
    """Generate a synthetic Bayer frame with stride width (2048)."""
    rng = np.random.default_rng(seed)
    return rng.random((1,1,h,w), dtype=np.float32)

# Load sessions
algo_sess = ort.InferenceSession("isp_algo_coeffs_stride_crop.onnx", providers=["CPUExecutionProvider"])
isp_sess  = ort.InferenceSession("isp_onnx_stride_crop.onnx", providers=["CPUExecutionProvider"])
...
for i in range(5):
    ...
    # Dynamic crop params: remove 64 pixels left/right → 1920 width
    crop_starts = np.array([0, 64], dtype=np.int64)
    crop_ends   = np.array([1080, 1984], dtype=np.int64)
    ...
    if "crop_starts" in pipeline_inputs:
        isp_inputs["crop_starts"] = crop_starts
    if "crop_ends" in pipeline_inputs:
        isp_inputs["crop_ends"] = crop_ends
    if "crop_axes" in pipeline_inputs:
        isp_inputs["crop_axes"] = crop_axes
    ...
    print(f"  CCM matrix shape: {ccm_next.shape}")
    print(f"  Lens coeffs: {lens_next.reshape(4)}")
